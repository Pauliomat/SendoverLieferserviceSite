...
function printData(data) { 
    ...
    for(var i = 0; i < len; i++){
        ...
        legend.appendChild(veggieImg);
        veggieImg.width = "30";
        veggieImg.height = "30";

        if(data[i]["vegetarisch"] == "true" && data[i]["vegan"] == "false") {
            veggieImg.src = "../../pics/vegetarisch.jpg";
        } else if(data[i]["vegan"] == "true") {
            veggieImg.src = "../../pics/vegan.png";
        } else {
            legend.removeChild(veggieImg);
        }

        //img
        var img = document.createElement("img");

        if(typeof(data[i]["bild"])=="object"){
            var bild64 = pictureToBase64(data[i]["bild"].data)
            img.src = "data:image/png;base64," + bild64;
        } else {
            img.src = data[i]["bild"];
        }
        img.id = "item_img";
